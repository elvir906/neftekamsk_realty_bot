...
from .utils import Output, vk_club_ids
...
class message_texts():
    on = {
        ...
        'about': [
                ...
                + 'Статистика пользования ботом доступна по команде /getstatistics\n\n',
                'Последнее обновление: 20.04.2023\n\n',

                '*Версия 1.15:*\n'
                + '- Автопост (бета-версия) объектов в ВК;\n',
        ...
        'vk_autopost_instruction':
        '❗❗❗*ВНИМАНИЕ!*❗❗❗\n\n'
        + '1. Эта функция на стадии тестирования и отладки. '
        + 'Одновременная отправка пока ограничена '
        + f'{AUTOPOST_LIMIT}-ю объектами (в дальнейшем, еcли будет '
        + 'всё OK, то лимит будут увеличивать).\nОтправка осуществляется на '
        + f'стену *{len(vk_club_ids)} групп*, по типу барахолок, *с открытыми стенами.*\n'
        + 'Каждая отправка идёт с интервалом в 45 сек. '
        + 'Хорошая партия постов займёт время. Пока идёт отправка '
        + 'ничего не трогай, не давай команды боту и даже не дыши '
        + 'в сторону бота. 🙂\n\n'
        + '2. В настройках аккаунта VK у тебя должна быть подключена '
        + 'двухфакторная аутентификация. Это обязательно! Возможно, во время '
        + 'постинга поступит звонок-сброс на номер, указанный как логин для VK. '
        + 'Просто ответьте на звонок. \n\n'
        + '3. *Каждый раз, при отправке, бот будет просить ввести '
        + 'логин и пароль от твоего аккаунта ВК, с которого ты обычно '
        + 'делаешь посты, потому что бот не запоминате и не хранит эти '
        + 'данные у себя.*\n\n'
        + '4. Так же для отправки пригодятся *резервные коды* '
        + 'двухфакторной аутентификации, их так же следует вводить вручную. '
        + 'Найти коды ты сможешь в VK ID или по ссылке '
        + 'https://id.vk.com/account/#/reserve-codes. \n\n'
        # + '5. Разработчик оставил за собой право разместить в '
        # + 'публикуемых через бот постах ссылку на ТГ-канал '
        # + '"Недвижимость города Нефтекамска" где публикуются объекты '
        # + 'пользователей бота.\n\n'
        + '5. *Политика ответственности:*\n'
        + '- за публикуемый через бот в соц.сети "ВКонтакте" материал '
        + 'ответственность несёт пользователь бота. Посты публикуются '
        + 'от имени пользователя, аутентифицированного через '
        + 'введённые логин и пароль;\n'
        + '- разработчик не несёт ответственности за возможную '
        + 'блокировку аккаунта пользователя;\n'
        + '- разработчик не несёт ответственности за возможный визуальный, '
        + 'или какой бы то ни было, перехват '
        + 'логина и пароля от  аккаунта VK третьими лицами при вводе их во '
        + 'время работы с ботом или в отсутствии пользователя. '
        + 'Будьте внимательны и очищайте чат с ботом.\n\n'
        + '6. Предусмотрена возможность внедрения в код бота индивидуального '
        + 'дизайна постов (добавление слоганов, пиктограмм, изменение текстов) '
        + 'за отдельную оплату. Расценки у @davletelvir\n\n'
        + '7. Пользуясь функцией "vk-autopost", ты соглашаешься'
        + 'с настоящими правилами и политикой ответственности.\n\n'
        + '❗❗ 👆 *ПРОЧИТАЙ, ЕСЛИ ПОЛЬЗУЕШЬСЯ ВПЕРВЫЕ* 👆 ❗❗\n\n'
        + 'И так, если ты прочитал (прочитала) правила выше и у тебя всё готово, то ...\n\n'
        + f'✏ *Выбери не более {AUTOPOST_LIMIT} объектов* для постинга и '
        + 'нажми на кнопку "💫 Подтвердить выбор":\n\n',

        'vk_adpost_instruction':
        '❗❗❗*ВНИМАНИЕ!*❗❗❗\n\n'
        '1. Эта функция предназначена для автопостинга рекламных '
        + 'сообщений на стенах сообществ.\n'
        + 'Отправка осуществляется на '
        + f'стену *{len(vk_club_ids)} групп*, по типу барахолок, *с открытыми стенами.*\n'
        + 'Каждая отправка идёт с интервалом в 45 сек. '
        + 'Хорошая партия постов займёт время. Пока идёт отправка '
        ...
        + 'дизайна постов (добавление слоганов, пиктограмм, изменение текстов) '
        + 'за отдельную оплату. Расценки у @davletelvir\n\n'
        + '7. Пользуясь функцией "vk_adpost", ты соглашаешься'
        + 'с настоящими правилами и политикой ответственности.\n\n'
        + '❗❗ 👆 *ПРОЧИТАЙ, ЕСЛИ ПОЛЬЗУЕШЬСЯ ВПЕРВЫЕ* 👆 ❗❗\n\n'
        + 'И так, если ты прочитал (прочитала) правила выше и у тебя всё готово, то ...\n\n'
    ...
    def entering_description_text(category: str) -> str:
        text = (
            ...
            + ' Не превышай это ограничение и не дублируй '
            + 'в описании информацию о квартире, такие как этаж, цена и др. '
            + '\n\nТолько самую суть - состояние. И если надо,'
    ...
    def statistics_text(data):
        text = (f'*Статистика пользования ботом на {str(dt.datetime.now().strftime("%d.%m.%Y"))}:*\n'
                ...
                + f'всего боту скомандовали *{data.get("command_counter")}* раз (-а).'
                )
        return text

    def aqidel():
        text = ('Помимо разработки я интересуюсь краеведением и историей. И вот моя\n'
                + '*гипотеза засыпанного русла* (части русла р.Белая, респ.Башкортостан).\n\n'
